...
function DayCell({
  ...
  return (
    ...
    >
      ...
      <span
        className={` leading-5 ${view === "compact" ? (initialState ? "text-sm" : " text-lg") : initialState ? "text-xl" : " text-3xl"}`}
      >
        {format(date, "d")}
      </span>
...
function MonthCalender({
  ...
  return (
    <div className="@container p-2 border  rounded">
      <h4
        className={`mb-2 font-medium leading-5 ${view === "compact" ? " text-lg" : " text-3xl mb-6"} `}
      >
      ...
      <div className="grid grid-cols-7 gap-[clamp(2px,0.5cqw,4px)] ">
        {weekDays.map((day) => (
          <div
            className={`text-center lg:h-12 bg-secondary rounded flex items-center justify-center text-lg  text-foreground border  ${view === "compact" ? "text-xs " : " text-lg"} `}
            key={day}
          >
            {day}
          </div>
...
export default function HabbitTrackerCalender({
  ...
  return (
    <>
      <div className="flex gap-1  border w-fit ml-auto mt-6 rounded p-0.5">
        <button
      ...
      <div
        className={
          view === "detailed" ? "grid grid-cols-1 gap-4" : "grid gap-3"
        }
        style={
          view === "compact"
